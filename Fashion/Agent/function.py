...
def Classify():
    model_path = "ClothingClassifier.pth"
    model = load_model(NeuralNetwork, model_path, device)
    imagePath = "" #input("Enter the path to the image you want to test: ") needs to be changed for agent
    img = preprocess_image(imagePath)
    logits = model(img.to(device))
    pred_probab = nn.Softmax(dim=1)(logits)
    y_pred = pred_probab.argmax(1)
    predClass = labels_map[y_pred.item()]
    return predClass
